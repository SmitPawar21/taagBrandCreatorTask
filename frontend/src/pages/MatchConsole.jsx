...
const MatchConsole = () => {
    ...
    console.log("brandbrief in match console: ", brandBrief);
    ...
    const getSelectedCreatorsData = () => {
        return creators.filter(item => selectedCreators.has(item.creator._id));
    };

    const getTotalCost = () => {
        return getSelectedCreatorsData().reduce((total, item) => total + item.creator.basePriceINR, 0);
    };
                    ...
                    <div className="max-w-6xl mx-auto mb-8">
                        <div className="bg-green-900/20 border border-green-700/50 rounded-2xl p-6">
                            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                <div>
                                    <h3 className="text-xl font-semibold mb-2" style={{ color: '#C5F37D' }}>
                                        Selected Creators ({selectedCreators.size})
                                    </h3>
                                    <p className="text-gray-300">
                                        Total estimated cost: <span className="font-bold text-white">{formatCurrency(getTotalCost())}</span>
                                    </p>
                                </div>
                                <button
                                    onClick={handleProceedToBilling}
                                    className="px-8 py-3 font-semibold rounded-xl transition-all duration-200 hover:scale-105 shadow-lg"
                                    style={{ backgroundColor: '#C5F37D', color: '#000' }}
                                >
